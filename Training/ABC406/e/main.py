...
for _ in [0] * T:
    N, K = il()
    M = max([i for i in range(61) if N & (1 << i)])
    dp = create_array3(M + 1, K + 1, 2, (0, 0))
    ...
    for i in range(M):
        for k in range(K + 1):
            if k + 1 > K:
                if N & (1 << (M - i - 1)):
                    dp[i + 1][k][0] = op(dp[i + 1][k][0], dp[i][k][1])
                else:
                    dp[i + 1][k][1] = op(dp[i + 1][k][1], dp[i][k][1])

                dp[i + 1][k][0] = op(dp[i + 1][k][0], dp[i][k][0])
            ...
            dp[i + 1][k + 1][0] = op(
                ...
                (
                    dp[i][k][0][0],
                    dp[i][k][0][1] + (1 << (M - i - 1)) * dp[i][k][0][0],
                ),
            )
            dp[i + 1][k][0] = op(dp[i + 1][k][0], dp[i][k][0])

    print((dp[M][K][0][1] + dp[M][K][1][1]) % 998244353)
