...
for i in range(N):
    ...
    for a in range(3):
        if len(D["M"][a]) and D["M"][a][0] > i or len(D["M"][a]) == 0:
            continue

        bisa = bisect.bisect_right(D["M"][a], i)

        for c in range(3):
            if len(D["X"][c]) and D["X"][c][-1] < i or len(D["X"][c]) == 0:
                continue

            bisb = len(D["X"][c]) - bisect.bisect_left(D["X"][c], i)
