...
for A in range(1, 9 * 14):
    dp = create_array3(A, A + 1, 2, 0)
    dp[0][0][1] = 1

    for i in range(len(L)):
        new_dp = create_array3(A, A + 1, 2, 0)

        for m in range(A):
            for d in range(A + 1):
                for n in range(10):
                    if d + n <= A:
                        t = (m + (10 ** (len(L) - i - 1) * n)) % A
                        new_dp[t][d + n][0] += dp[m][d][0]

                        if n < L[i]:
                            new_dp[t][d + n][0] += dp[m][d][1]
                        elif n == L[i]:
                            new_dp[t][d + n][1] += dp[m][d][1]
