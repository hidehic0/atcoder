...
for i in range(H)[::-1]:
    for k in range(W)[::-1]:
        ...
        if (i + k) % 2 == 0:
            ...
            if i + 1 < H:
                dp[i][k] = max(dp[i][k], dp[i + 1][k] + B[i + 1][k])

            if k + 1 < W:
                dp[i][k] = max(dp[i][k], dp[i][k + 1] + B[i][k + 1])
        else:
            dp[i][k] = INF

            if i + 1 < H:
                dp[i][k] = min(dp[i][k], dp[i + 1][k] - B[i + 1][k])

            if k + 1 < W:
                dp[i][k] = min(dp[i][k], dp[i][k + 1] - B[i][k + 1])
