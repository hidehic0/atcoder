...
for i in range(M):
    ...
    if cnt[L[-1][-1]] is not None:
        Q.append((cnt[L[-1][-1]], i))
...
while Q:
    ...
    if len(L[a]):
        if cnt[L[a][-1]] is None:
            cnt[L[a][-1]] = a
        else:
            Q.append((cnt[L[a][-1]], a))

    if len(L[b]):
        if cnt[L[b][-1]] is None:
            cnt[L[b][-1]] = b
        else:
            Q.append((cnt[L[b][-1]], b))

YN(not len([l for l in L if len(l) != 0]))
