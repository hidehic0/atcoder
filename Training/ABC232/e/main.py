...
dp = create_array2(2, 2, 0)
dp[sx == gx][sy == gy] = 1

for _ in [0] * K:
    next = create_array2(2, 2, 0)
    next[0][0] = dp[0][0] * (H - 2 + W - 2) + (H - 1) * dp[1][0] + (W - 1) * dp[0][1]
    next[1][0] = dp[1][0] * (W - 2) + dp[0][0] + dp[1][1] * (W - 1)
    next[0][1] = dp[0][1] * (H - 2) + dp[0][0] + dp[1][1] * (H - 1)
    next[1][1] = dp[1][0] + dp[0][1]
