...
for i in range(24):
    p *= mod_div(100 - (X if S[i] == "T" else Y), 100, MOD)
    p %= MOD

skip = pow(1 + MOD - p, -1, MOD)
A = [[0] for _ in [0] * 23] + [[1]]
B = create_array2(24, 24, 1)
...
for i in range(24):
    for j in range(24):
        for k in range(1, 25):
            ...
            if nj != i:
                B[i][j] *= mod_div(100 - (X if S[nj] == "T" else Y), 100, MOD)
                B[i][j] %= MOD
            else:
                B[i][j] *= mod_div((X if S[nj] == "T" else Y), 100, MOD)
                B[i][j] %= MOD
                break
...
C = mat_mul(mat_pow(B, N, MOD), A, MOD)
