...
mat_a = [[1]] + [[0] for _ in [0] * ((1 << H) - 1)]
mat_b = [[0] * (1 << H) for _ in [0] * (1 << H)]

used = nb = 0


def rec(bit, ind):
    global nb, used, mat_b
    if ind == H:
        mat_b[nb][bit] += 1
        return
    ...
    if ind + 1 < H and not (used >> (ind + 1)) & 1:
        used ^= 1 << ind
        used ^= 1 << (ind + 1)
        rec(bit, ind + 1)
        used ^= 1 << (ind + 1)
...
print(mat_mul(mat_pow(mat_b, W, 998244353), mat_a, 998244353)[0][0])
