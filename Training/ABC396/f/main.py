...
seg = DualSegmentTree(lambda a, b: a + b, 0, N + 1)
_seg = SegTree(lambda a, b: a + b, 0, M + 1)
...
for i, a in reversed(list(enumerate(A))):
    ...
    ans += b
    seg.apply(i, i + 1, b)
    _seg.set(a, _seg.get(a) + 1)

for k in range(M):
    n = len(D[M - k])
    for i, ind in enumerate(D[M - k]):
        b = seg.get(ind)
        ans -= b
        seg.apply(ind, ind + 1, -b - (n - i + 1))
        seg.apply(0, ind, 1)
        ans += ind - i
