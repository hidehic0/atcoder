...
def dfs1(cur, par=-1):
    ...
    for nxt, w in G[cur]:
        ...
        L[cur].append((dfs1(nxt, cur) + w, nxt, w))
        res = max(res, L[cur][-1][0])

    L[cur].sort(reverse=True)
...
def dfs2(cur, par=-1, t=0):
    ans[cur] = t

    if len(L[cur]):
        ans[cur] = max(ans[cur], L[cur][0][0])

    for v, nxt, w in L[cur]:
        if nxt == par:
            continue

        if len(L[cur]) >= 2:
            if L[cur][0][0] == v:
                dfs2(nxt, cur, max([t, D[cur], L[cur][1][0]]) + w)
            else:
                dfs2(nxt, cur, max([t, D[cur], L[cur][0][0]]) + w)
        else:
            dfs2(nxt, cur, max(t, D[cur]) + w)
