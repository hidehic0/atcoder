...
for i in range(N):
    for k in range(N):
        a, b, c = L[i]
        p, q, r = L[k]
        dist[i][k] = abs(p - a) + abs(q - b) + max(0, r - c)

dp = create_array2(1 << N, N, INF)
dp[0][0] = 0
...
for bit in range(1 << N):
    for cur in range(N):
        for nxt in range(N):
            ...
            nbit = bit | 1 << nxt
            dp[nbit][nxt] = min(dp[nbit][nxt], dp[bit][cur] + dist[cur][nxt])

print(min([dp[-1][i] + dist[i][0] for i in range(N)]))
