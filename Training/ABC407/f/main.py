...
seg1 = SegTree(lambda a, b: max(a, b), 0, A)
seg2 = DualSegmentTree(lambda a, b: a + b, 0, N + 1)

for i in range(N):
    cnt_l = (
        i
        - binary_search(lambda mid: seg1.prod(mid, i) >= A[i], i, -1, return_left=False)
        + 1
    )
    cnt_r = binary_search(lambda mid: seg1.prod(i, mid + 1) <= A[i], N, i) - i

    seg2.apply(0, cnt_l, A[i])
    seg2.apply(cnt_r + 1, cnt_r + cnt_l + 1, -A[i])
