...
N = ii()
P = il(-1)
E = [(P[i - 1], i, 1) for i in range(1, N)]
...
for i in range(N):
    D[ET.path_query_from_root(i)].append(ET.first_arrival[i])
...
for _ in [0] * Q:
    ...
    if ET.path_query_from_root(u) > d:
        print(0)
        continue

    bis_r = bisect.bisect_right(D[d], ET.last_arrival[u]) - 1

    left, right = -1, len(D[d])

    while right - left > 1:
        mid = (left + right) // 2

        if D[d][mid] < ET.first_arrival[u]:
            left = mid
        else:
            right = mid
