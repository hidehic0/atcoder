...
def rec(n):
    ...
    for x in calc_divisors(n):
        ...
        if len(naa) == 0 and len(nab) == 0:
            ...
            if len(naa) - 2 > len(nab):
                na = f"({nab})"
            else:
                na = naa

        if len(nba) == 0 and len(nbb) == 0:
            continue
        elif len(nba) == 0:
            nb = f"({nbb})"
        elif len(nbb) == 0:
            ...
            if len(nba) - 2 > len(nbb):
                nb = f"({nbb})"
            else:
                nb = nba

        a.set((len(f"{na}*{nb}"), f"{na}*{nb}"))

    for x in range(1, n):
        y = n - x

        na, nb = ChangeMin((INF, "")), ChangeMin((INF, ""))
