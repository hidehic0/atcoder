...
if D == 0:
    print(sum(max(x - 1, 0) for x in L.values()))
    exit()


def solve(lis):
    dp = create_array2(len(lis) + 1, 2, INF)  # 0が未確定 1が確定済み
    dp[0][1] = 0

    for i in range(len(lis)):
        dp[i + 1][0] = dp[i][1]  # 確定済みでないと飛ばしてしまうため
        dp[i + 1][1] = min(dp[i][0], dp[i][1]) + L[lis[i]]  # どっちでもええで
