...
def cumulative_max(A):
    res = [0]

    for i in range(N - 1):
        res.append(max(res[-1], A[i]))
...
for i in range(N - 1):
    ...
    bis_a = binary_search(lambda x: CA[x] <= A[i], N, i + 1)
    bis_b = binary_search(lambda x: CB[x] <= A[i], N, 0)
    bis_a -= i
...
for i in range(N - 1):
    ...
    bis_a = binary_search(lambda x: CB[x] <= B[i], N, i + 1)
    bis_b = binary_search(lambda x: CA[x] < B[i], N, 0)
    bis_a -= i
