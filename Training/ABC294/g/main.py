...
def dfs_1(cur, root, d=0):
    global euler_tour

    for nxt, w in G[cur]:
        if nxt != root:
            euler_tour.append(nxt)
            edge_cost.append(w)
            depth.append(d + 1)
            dfs_1(nxt, cur, d + 1)
            euler_tour.append(cur)
            edge_cost.append(-w)
            depth.append(d)
...
last_arrival = [-1] * N
first_arrival[0] = 0
last_arrival[0] = len(euler_tour) - 1

edge_plus = [-1] * (N - 1)
edge_minus = [-1] * (N - 1)

for i in range(1, len(euler_tour)):
    edge_id = D[(euler_tour[i - 1], euler_tour[i])]

    if edge_cost[i] >= 0:
        edge_plus[edge_id] = i
    else:
        edge_minus[edge_id] = i

    if first_arrival[euler_tour[i]] == -1:
        first_arrival[euler_tour[i]] = i

    last_arrival[euler_tour[i]] = i

seg_1 = SegTree(lambda a, b: a + b, 0, edge_cost)
seg_2 = SegTree(min, (INF, INF), [(d, i) for i, d in enumerate(depth)])
...
for _ in [0] * Q:
    ...
    if l[0] == 1:
        ...
        seg_1.set(edge_plus[i], w)
        seg_1.set(edge_minus[i], -w)
    else:
        u, v = map(lambda x: x - 1, l[1:])
        ...
        l, r = (
            min(first_arrival[u], first_arrival[v]),
            max(last_arrival[u], last_arrival[v]),
        )
        lca = seg_2.prod(l, r)[1]

        u_w = seg_1.prod(0, first_arrival[u] + 1)
        v_w = seg_1.prod(0, first_arrival[v] + 1)
        try:
            lca_w = seg_1.prod(0, lca + 1)
        except:
            lca_w = 0

        print(u_w + v_w - 2 * lca_w)
