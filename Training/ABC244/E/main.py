...
S, T, X = S - 1, T - 1, X - 1
dp = [[[0, 0] for _ in [0] * N] for _ in [0] * (K + 1)]
dp[0][S][0] = 1  # dp配列の初期化

# 貰うdp(joiの切符ももらいたい(地域ブロックで))
for i in range(1, K + 1):
    for current in range(N):
        for back in G.get(current):
            if current == X:
                dp[i][current][0] += dp[i - 1][back][1]
                dp[i][current][1] += dp[i - 1][back][0]
            else:
                dp[i][current][0] += dp[i - 1][back][0]
                dp[i][current][1] += dp[i - 1][back][1]
