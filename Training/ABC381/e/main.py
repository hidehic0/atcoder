...
while Q:
    L, R = il(-1)
    bis_l = bisect.bisect_left(A3, L)
    bis_r = bisect.bisect_right(A3, R)

    if bis_l >= len(A3) or bis_r < 0 or bis_l > bis_r:
        print(0)
        Q -= 1
        continue
    ...
    def f(mid):
        ...
        res = max(2 * min(a, b) + 1, res)

        return b - a >= 0

    slash_point = binary_search(f, bis_r, bis_l - 1)
