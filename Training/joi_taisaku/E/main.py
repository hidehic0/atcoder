...
for i in range(N):
    if seted[i] != -1:
        match seted[i]:
            ...
            case 1:
                dp[i + 1][2] = dp[i][0] + dp[i][1] + dp[i][3]

            case 2:
                dp[i + 1][3] = dp[i][0] + dp[i][1] + dp[i][2]
    else:
        dp[i + 1][1] = dp[i][2] + dp[i][3] + 1
        dp[i + 1][2] = dp[i][1] + dp[i][3] + 1
        dp[i + 1][3] = dp[i][1] + dp[i][2] + 1

    dp[i + 1] = [k % 10000 for k in dp[i + 1]]
