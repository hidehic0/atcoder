...
while PQ:
    ...
    for nxt, l, d, k, c in G[cur]:
        if l > cos - c:
            continue

        ncos = l + d * binary_search(lambda x: l + d * x <= cos - c, k, 0)

        if ncos > used[nxt]:
            used[nxt] = ncos
            heapq.heappush(PQ, (-ncos, nxt))

print(*["Unreachable" if i == -INF else i for i in used[:-1]], sep="\n")
