...
P1, P2 = 98975190497478859, 57379907003781773

hasha_1, hasha_2, hashb_1, hashb_2 = [0], [0], [0], [0]

for a in A:
    hasha_1.append((pow(N, a, P1) + hasha_1[-1]) % P1)
    hasha_2.append((pow(N, a, P2) + hasha_2[-1]) % P2)
for b in B:
    hashb_1.append((pow(N, b, P1) + hashb_1[-1]) % P1)
    hashb_2.append((pow(N, b, P2) + hashb_2[-1]) % P2)

for _ in [0] * Q:
    l, r, L, R = il()

    YN(
        (hasha_1[r] - hasha_1[l - 1]) % P1 == (hashb_1[R] - hashb_1[L - 1]) % P1
        and (hasha_2[r] - hasha_2[l - 1]) % P2 == (hashb_2[R] - hashb_2[L - 1]) % P2
    )
