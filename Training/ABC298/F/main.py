...
N = ii()
L = li(N, il)
x_map, y_map = compress_2d([l[:-1] for l in L])

seg = SegTree(max, 0, len(y_map))
H = defaultdict(list)
SH = defaultdict(int)

for i in range(N):
    x, y, t = x_map[L[i][0]], y_map[L[i][1]], L[i][2]
    H[x].append((y, t))
    SH[x] += t
    seg.set(y, seg.get(y) + t)
