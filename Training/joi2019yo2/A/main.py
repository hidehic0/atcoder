...
# コード
N = ii()
S = [list(s()) for _ in [0] * N]
T = [list(s()) for _ in [0] * N]
...
R1 = [list(reversed(items)) for items in zip(*S)]

ans = min(ans, GetScore(R1) + 1)

R2 = [list(items) for items in zip(*map(reversed, S))]

ans = min(ans, GetScore(R2) + 1)

R3 = [list(reversed(items)) for items in zip(*R1)]

ans = min(ans, GetScore(R3) + 2)
