...
def op(a, b):
    ...
    if a[0] == -1:
        res[0] = b[0]
    res[2] = (a[2] * pow(e, b[1], mod)) % mod + b[2]
    res[3] = (b[3] * pow(e, a[1], mod)) % mod + a[3]
    res[2] %= mod
    res[3] %= mod
...
seg = SegTree(
    op, [-1, 0, 0, 0], [[i, 1, ord(t) - 97, ord(t) - 97] for i, t in enumerate(S)]
)
...
for _ in [0] * Q:
    ...
    if l[0] == "1":
        k = ord(l[2]) - 97
        seg.set(int(l[1]) - 1, [int(l[1]) - 1, 1, k, k])
    else:
        l, r = map(int, l[1:])

        _, _, a, b = seg.prod(l - 1, r)
