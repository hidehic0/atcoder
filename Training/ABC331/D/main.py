...
for _ in [0] * Q:
    ...
    hc = ((C - A + 1) - (N - (A % N)) - (C % N + 1)) // N
    wc = ((D - B + 1) - (N - (B % N)) - (D % N + 1)) // N
    A %= N
    B %= N
    C %= N
    D %= N

    res = ps.prod(0, 0, N, N) * (hc * wc)
    res += ps.prod(A, B, N, N)
    res += ps.prod(0, B, C + 1, N)
    res += ps.prod(0, 0, C + 1, D + 1)
    res += ps.prod(A, 0, N, D + 1)

    res += ps.prod(0, B, N, N) * hc
    res += ps.prod(0, 0, N, D + 1) * hc
    res += ps.prod(A, 0, N, N) * wc
    res += ps.prod(0, 0, C + 1, N) * wc
