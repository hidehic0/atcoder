...
D = [-1] * (N + 1)
RD = [-1] * (N + 1)
for ind, (x, y, i) in enumerate(sorted(L, key=lambda x: (x[1], x[0]))):
    D[i] = ind
    RD[ind] = i
...
for x, y, i in sorted(L, key=lambda x: (x[0], x[1]))[1:]:
    tmp = seg.prod(0, D[i]) + 1
    seg.set(D[i], tmp)
...
ans = ["D" * (H - L[RD[cur]][0]) + "R" * (W - L[RD[cur]][1])]
i = cur - 1

while cnt > 0:
    if (
        cnt == seg.get(i)
        and L[RD[i]][0] <= L[RD[cur]][0]
        and L[RD[i]][1] <= L[RD[cur]][1]
    ):
        ans.append(
            "D" * (L[RD[cur]][0] - L[RD[i]][0]) + "R" * (L[RD[cur]][1] - L[RD[i]][1])
        )
        cur = i
        cnt -= 1
...
ans.append(
    "D" * (H - "".join(ans).count("D") - 1) + "R" * (W - "".join(ans).count("R") - 1)
)
