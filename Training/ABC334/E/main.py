...
for x in range(H):
    for y in range(W):
        ...
        for nx, ny in grid_moves(x, y, H, W, MOVES, lambda a, b: S[a][b] == "#"):
            UF.unite(L[x][y], L[nx][ny])
...
for x in range(H):
    for y in range(W):
        ...
        for nx, ny in grid_moves(x, y, H, W, MOVES):
            if S[nx][ny] == "#":
                t.add(UF.root(L[nx][ny]))
