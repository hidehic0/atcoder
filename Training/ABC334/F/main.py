...
seg = SegTree(min, INF, N + 1)
seg.set(0, 0)
ans = euclid_dis(*L[0], sx, sy) ** 0.5

for i in range(1, N):
    ans += euclid_dis(*L[i - 1], *L[i]) ** 0.5

for i in range(1, N + 1):
    seg.set(
        i,
        seg.prod(max(0, i - K), i)
        + (
            (euclid_dis(*L[i - 1], sx, sy) ** 0.5)
            + (euclid_dis(*L[i], sx, sy) ** 0.5)
            - (euclid_dis(*L[i], *L[i - 1]) ** 0.5)
        ),
    )

ans += seg.get(N)

ans += euclid_dis(*L[N - 1], sx, sy) ** 0.5
