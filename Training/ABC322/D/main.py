...
def compress_2d(points):
    """
    二次元座圧を行う関数
    points: [(x1, y1), (x2, y2), ...] の形式の座標リスト
    戻り値: 座圧後の座標リスト、x座標のマッピング、y座標のマッピング
    """
    # x座標とy座標を別々に取得
...
for S in L:
    ...
    NL.append(compress_2d(T))

D = [[l] for l in NL]

for i in range(3):
    for _ in range(3):
        D[i].append(turn(D[i][-1]))
...
for a in range(16):
    for b in range(16):
        for c in range(16):
            MOVES_ID.append((a, b, c))


for a in range(4):
    for b in range(4):
        for c in range(4):
            for moveid in MOVES_ID:
                MOVES = [ST[moveid[0]], ST[moveid[1]], ST[moveid[2]]]
                M = [a, b, c]
                ...
                for i in range(3):
                    ...
                    for x, y in D[i][M[i]]:
                        if (
                            coordinate_check(mx + x, my + y, 4, 4)
                            and (mx + x, my + y) not in L
                        ):
                            L.add((mx + x, my + y))
                        else:
                            break
