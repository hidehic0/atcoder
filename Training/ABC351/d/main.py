...
for i in range(H):
    for k in range(W):
        ...
        for nx, ny in grid_moves(i, k, H, W):
            magnets.add((nx, ny))


def c(x, y):
    return S[x][y] != "#" and not used[x][y]
...
for i in range(H):
    for k in range(W):
        ...
        while Q:
            ...
            for nx, ny in grid_moves(x, y, H, W, [(0, 1), (0, -1), (1, 0), (-1, 0)], c):
                if (nx, ny) in magnets:
                    T.add((nx, ny))
                    continue
