...
for i in range(H):
    for k in range(W):
        if not (
            (k > 0 and A[i][k] == A[i][k - 1]) or (k + 1 < W and A[i][k] == A[i][k + 1])
        ):
            L[i].append(k)

dp = create_array3(H, 2, 2, INF)
        ...
        for b in range(2):
            for c in range(2):
                for k in L[i]:
                    if (
                        A[i - 1][k] ^ a != A[i][k] ^ b
                        and A[i][k] ^ b != A[i + 1][k] ^ c
                    ):
                        break
                else:
                    dp[i + 1][b][c] = min(dp[i + 1][b][c], dp[i][a][b] + c)
...
print(-1 if ans.val() == INF else ans.val())
