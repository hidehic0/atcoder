...
class State:
    def __init__(self, score, B, L):
        """
        warning: 引数のscoreは、すでにスコアが取られているのが前提
        """
        self.score = score
        self.B = [l[:] for l in B]
        self.L = [l[:] for l in L]
    ...
    def copy(self):
        return State(self.score, self.B, self.L)


cur = State(getScore(A), [l[:] for l in A], [])
    ...
    for m in range(M):
        for p in range(N - 2):
            for q in range(N - 2):
                ...
                for i in range(3):
                    for j in range(3):
                        sta.B[i + p][j + q] += S[m][i][j]
                        sta.B[i + p][j + q] %= MOD

                sta.score = getScore(sta.B)
                sta.L.append((m, p, q))

                if ns.score < sta.score:
                    ns = sta.copy()

    if cur.score < ns.score:
        cur = ns.copy()
