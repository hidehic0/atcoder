...
seg = SegTree(lambda a, b: max(a, b), 0, N + 1)

for l in sorted(D.items(), key=lambda x: x[0], reverse=True):
    for w, d in l[1]:
        YE(seg.prod(wmap[w] + 1, N + 1) > d)

    for w, d in l[1]:
        seg.set(wmap[w], max(seg.get(wmap[w]), d))
