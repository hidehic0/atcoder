...
ans = [M] * Q
seg = SegTree(lambda a, b: a + b, 0, 2 * N)

i = 0

for a, b, ind in L:
    while i < M and E[i][1] > b:
        seg.set(E[i][0], seg.get(E[i][0]) + 1)
        i += 1

    ans[ind] -= seg.prod(0, a)
...
for a, b, ind in L:
    ...
    ans[ind] -= seg.prod(a, b + 1)

E.sort(key=lambda x: x[0])

for a, b, ind in L:
    bis = bisect.bisect_left(E, b, key=lambda x: x[0])
    ans[ind] -= M - bis

E.sort(key=lambda x: x[1])

for a, b, ind in L:
    bis = bisect.bisect_left(E, a, key=lambda x: x[1])
    ans[ind] -= bis
