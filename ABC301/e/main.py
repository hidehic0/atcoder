...
def dist(sx: int, sy: int):
    ...
    while Q:
        ...
        for nx, ny in grid_moves(x, y, H, W, MOVES1, lambda x, y: L[x][y] != "#"):
...
for i in range(M):
    if D[sx][sy][S[i][0]][S[i][1]] != INF:
        dp[1 << i][i] = D[sx][sy][S[i][0]][S[i][1]]
...
for bit in range(1 << M):
    for cur in range(M):
        ...
        for nxt in range(M):
            if bit & (1 << nxt):
                continue
            d = D[S[cur][0]][S[cur][1]][S[nxt][0]][S[nxt][1]]
            if d == INF:
                continue

            dp[bit | (1 << nxt)][nxt] = min(dp[bit | (1 << nxt)][nxt], dp[bit][cur] + d)
...
for bit in range(1 << M):
    for i in range(M):
        ...
        if D[S[i][0]][S[i][1]][gx][gy] + dp[bit][i] <= T:
