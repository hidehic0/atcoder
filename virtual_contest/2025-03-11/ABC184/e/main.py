...
S = li(H, s)
D = defaultdict(list)
ItC, CtI = coordinates_to_id(H, W)
G = Graph(H * W, dire=True)
...
for i in range(H):
    for k in range(W):
        ...
        for nx, ny in grid_moves(
            i, k, H, W, [(0, 1), (0, -1), (1, 0), (-1, 0)], lambda x, y: S[x][y] != "#"
        ):
            G.new_side(CtI[i][k], CtI[nx][ny])

        if S[i][k] in lowerlist:
            D[S[i][k]].append(CtI[i][k])
...
while Q:
    ...
    for nxt in G.get(cur):
        if used[cur] + 1 < used[nxt]:
            used[nxt] = used[cur] + 1
            Q.append((nxt, used[nxt]))

    x, y = ItC[cur]

    if S[x][y] in lowerlist:
        for nxt in D[S[x][y]]:
            if used[cur] + 1 < used[nxt]:
                used[nxt] = used[cur] + 1
                Q.append((nxt, used[nxt]))
...
print(used[gi] if used[gi] != INF else -1)
