...
def check(AHA, BHA):
    al, ar = AHA.get(0, i + 1), AHA.get(i + 1, N)
    bl, br = BHA.get(N - i - 1, N), BHA.get(0, N - i - 1)

    return al == bl and ar == br


for _ in [0] * T:
    A = input().replace("0", "a").replace("1", "b")
    B = input().replace("0", "a").replace("1", "b")
    N = len(A)
    ...
    AHA, AHB, AHC = RollingHash(A), RollingHash(A, 998244353), RollingHash(A, 10**9 + 7)
    BHA, BHB, BHC = RollingHash(B), RollingHash(B, 998244353), RollingHash(B, 10**9 + 7)

    for i in range(N - 1):
        if check(AHA, BHA) and check(AHB, BHB) and check(AHC, BHC):
            print(i + 1)
            break
    else:
        print(-1)
