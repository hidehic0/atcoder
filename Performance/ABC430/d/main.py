...
N = ii()
X = [0] + il()
P = sorted([(x, i) for i, x in enumerate(X)])
...
for i in range(1, N + 1):
    nd = X[i] - L[L.bisect_left((X[i], i)) - 1][0]

    if L[-1][0] > X[i]:
        nd = min(L[L.bisect_left((X[i], i))][0] - X[i], nd)
    ...
    bis = L.bisect_left((X[i], i))

    lx, li = L[bis - 1]

    if li < i:
        if X[i] - lx < D[li] or D[li] == 0:
            cur -= D[li]
            D[li] = X[i] - lx
            cur += D[li]
