...
for _ in [0] * T:
    ...
    bdp = create_array2(N, N, INF)

    for i in range(N):
        bdp[-1][i] = (A[-1][i - 1] if i - 1 >= 0 else 0) + (
            A[-1][i + 1] if i + 1 < N else 0
        )

    for i in range(N - 1)[::-1]:
        for k in range(N):
            b = (A[i][k - 1] if k - 1 >= 0 else 0) + (A[i][k + 1] if k + 1 < N else 0)

            if k - 1 >= 0:
                bdp[i][k] = min(bdp[i][k], bdp[i + 1][k - 1] + b)
            if k + 1 < N:
                bdp[i][k] = min(bdp[i][k], bdp[i + 1][k + 1] + b)

    udp = create_array2(N, N, INF)

    for k in range(N):
        udp[0][k] = (A[0][k - 1] if k - 1 >= 0 else 0) + (
            A[0][k + 1] if k + 1 < N else 0
        )
    ...
    for i in range(1, N):
        for k in range(N):
            ...
            if k - 1 >= 0:
                udp[i][k] = min(udp[i][k], udp[i - 1][k - 1] + b)
            if k + 1 < N:
                udp[i][k] = min(udp[i][k], udp[i - 1][k + 1] + b)

    ans = create_array2(N, N, INF)
    ...
    for i in range(N):
        for k in range(N):
            for t in [k - 1, k + 1]:
                ...
                b = (A[i][t - 1] if t - 1 >= 0 else 0) + (
                    A[i][t + 1] if t + 1 < N else 0
                )

                ans[i][k] = min(ans[i][k], bdp[i][t] + udp[i][t] - b)
