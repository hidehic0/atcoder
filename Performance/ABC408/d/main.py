...
while Q:
    ...
    dp = create_array3(N + 1, 2, 2, INF)
    dp[0][0] = [0, 0]

    for i in range(N):
        dp[i + 1][0][0] = min(dp[i + 1][0][0], dp[i][0][0] + int(S[i] == "1"))
        dp[i + 1][0][1] = min(dp[i + 1][0][1], dp[i][0][1] + int(S[i] == "1"))
        dp[i + 1][1][0] = min(
            dp[i + 1][1][0], min(dp[i][0][0], dp[i][1][0]) + int(S[i] == "1")
        )
        dp[i + 1][1][1] = min(
            dp[i + 1][1][1], min(dp[i][0][0], dp[i][1][1]) + int(S[i] == "0")
        )
        dp[i + 1][0][1] = min(dp[i + 1][0][1], dp[i][1][1] + int(S[i] == "1"))

    print(min(min(dp[-1][0]), min(dp[-1][1])))
