...
while Q:
    ...
    if tp == 0:
        for nx, ny in grid_moves(x, y, H, W, MOVES1):
            if A[nx][ny] == "#" or used_grid[nx][ny] != -1:
                continue

            used_grid[nx][ny] = used_grid[x][y] + 1
            Q.append((0, nx, ny))

        if A[x][y] not in (".", "#"):
            if used_warp[ord(A[x][y]) - 97] == -1:
                used_warp[ord(A[x][y]) - 97] = used_grid[x][y]
                Q.appendleft((1, ord(A[x][y]) - 97, -1))
    else:
        for nx, ny in D[x]:
            if used_grid[nx][ny] == -1:
                used_grid[nx][ny] = used_warp[x] + 1
                Q.append((0, nx, ny))
