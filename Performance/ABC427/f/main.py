...
TA = [N // 4] * 2
TB = [N // 4] + [N // 4 + (N % 4)]
DAA, DBA = defaultdict(int), defaultdict(int)

na, nb = TA

for bita in range(1 << na):
    if bita.bit_count() > na // 2 + 1:
        continue
    ...
    for i in range(na):
        ...
        for bitb in range(1 << nb):
            if bitb.bit_count() > nb // 2 + 1:
                continue

            if bita & (1 << (na - 1)) and bitb & (1 << 0):
                continue
...
for bitc in range(1 << nc):
    if bitc.bit_count() > nc // 2 + 1:
        continue
    ...
    for i in range(nc):
        ...
        for bitd in range(1 << nd):
            if bitd.bit_count() > nd // 2 + 1:
                continue

            for i in range(nd):
                if bitd & (1 << i):
                    if prev + 1 == i + na + nb + nc:
                        break
                    prev = i + na + nb + nc
                    cur += A[i + na + nb + nc]
                    cur %= M
