...
dp = create_array3(K + 1, K + 1, 2, -INF)
dp[0][0][0] = 0

for i in range(N):
    ndp = create_array3(K + 1, K + 1, 2, -INF)

    for a in range(K + 1):
        for b in range(a + 1):
            for c in range(2):
                ndp[a][b][c] = max(ndp[a][b][c], dp[a][b][c])

                if a < K:
                    ndp[a + 1][b][c ^ 1] = max(ndp[a + 1][b][c ^ 1], dp[a][b][c])
                if a - b > 0:
                    ndp[a][b + 1][c ^ 1] = max(ndp[a][b + 1][c ^ 1], dp[a][b][c])
