...
for _ in [0] * N:
    ...
    if C == "X":
        for lx, rx, ly, ry in L:
            if rx < A:
                NL.append((lx, rx, ly - B, ry - B))
            elif lx >= A:
                NL.append((lx, rx, ly + B, ry + B))
            else:
                NL.append((lx, A - 1, ly - B, ry - B))
                NL.append((A, rx, ly + B, ry + B))
    else:
        for lx, rx, ly, ry in L:
            if ry < A:
                NL.append((lx - B, rx - B, ly, ry))
            elif ly >= A:
                NL.append((lx + B, rx + B, ly, ry))
            else:
                NL.append((lx - B, rx - B, ly, A - 1))
                NL.append((lx + B, rx + B, A, ry))
...
for i in range(len(L)):
    for k in range(i + 1, len(L)):
        a = L[i][0] <= L[k][0] <= L[i][1] or L[k][0] <= L[i][0] <= L[k][1]
        b = L[i][2] <= L[k][2] <= L[i][3] or L[k][2] <= L[i][2] <= L[k][3]

        if a and (L[i][3] + 1 == L[k][2] or L[k][3] + 1 == L[i][2]):
            UF.merge(i, k)
        if b and (L[i][1] + 1 == L[k][0] or L[k][1] + 1 == L[i][0]):
            UF.merge(i, k)
