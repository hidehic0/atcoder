...
# ユニークな文字があれば、K=1になる
# 各操作は、ある区間のひとを、ここに移動するという操作に言いかえられる
# bit dpから落せねえ...
...
for _ in [0] * N:
    for l in D.values():
        for k in range(len(l)):
            ...
            if p > i:
                t = seg.prod(p + 1, N) + seg.prod(0, i + 1)
                seg.apply(p + 1, N, 0)
                seg.apply(0, i + 1, 0)
            else:
                t = seg.prod(p + 1, i + 1)
            ...
            # print(l, *[seg.get(i) for i in range(N)])
            pass

print(len([None for i in range(N) if seg.get(i) == 1]))
# print(len(D) * N)
# print("".join(list(D.keys()) * N))
