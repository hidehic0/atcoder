...
for i in range(2 * N - 1):
    A.append(X[i] * Y[i + 1] - X[i + 1] * Y[i] + A[-1])
    B.append((X[i] + X[i + 1]) * (X[i] * Y[i + 1] - X[i + 1] * Y[i]) + B[-1])
    C.append((Y[i] + Y[i + 1]) * (X[i] * Y[i + 1] - X[i + 1] * Y[i]) + C[-1])
...
for _ in [0] * Q:
    ...
    a = A[v] - A[u] + X[v] * Y[u] - X[u] * Y[v]
    b = B[v] - B[u] + (X[v] + X[u]) * (X[v] * Y[u] - X[u] * Y[v])
    c = C[v] - C[u] + (Y[v] + Y[u]) * (X[v] * Y[u] - X[u] * Y[v])
