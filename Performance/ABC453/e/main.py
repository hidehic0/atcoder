...
for i in range(1,N):
    ...
    if (k,i) in D:
        if A[i]-D[(k,i)]<=i and A[k]-D[(k,i)]<=k and A[i]+A[k]-D[(k,i)]==N:
            t=i-(A[i]-D[(k,i)])
            n=D[(k,i)]
            ans+=mod_div(mod_div(P[n],P[t],998244353),P[n-t],998244353)
            ans%=998244353
        continue
    ...
    D[(i,k)]=ft.sum(k,N)
    if A[i]-D[(i,k)]<=i and A[k]-D[(i,k)]<=k and A[i]+A[k]-D[(i,k)]==N:
        t=i-(A[i]-D[(i,k)])
        n=D[(i,k)]
        ans+=mod_div(mod_div(P[n],P[t],998244353),P[n-t],998244353)
        ans%=998244353
