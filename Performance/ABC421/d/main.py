...
def h_check(tx, ty, ax, ay, ao, oc):
    global ans

    if ao == "L":
        if tx > ax and ty < ay and ay - ty == tx - ax and tx - ax <= oc:
            ans += 1
    elif ao == "R":
        if tx > ax and ty > ay and ty - ay == tx - ax and tx - ax <= oc:
            ans += 1
    elif ao == "D":
        if ty == ay and tx > ax and (tx - ax) % 2 == 0 and (tx - ax) // 2 <= oc:
            ans += 1


def w_check(tx, ty, ax, ay, ao, oc):
    global ans

    if ao == "U":
        if tx < ax and ay < ty and ty - ay == ax - tx and ax - tx <= oc:
            ans += 1
    elif ao == "D":
        if tx > ax and ay < ty and ty - ay == tx - ax and tx - ax <= oc:
            ans += 1
    elif ao == "R":
        if tx == ax and ty > ay and (ty - ay) % 2 == 0 and (ty - ay) // 2 <= oc:
            ans += 1


while len(TM) and len(AM):
    oc = min(TM[-1][1], AM[-1][1])
    to, ao = TM[-1][0], AM[-1][0]

    if to == ao and (tx, ty) == (ax, ay):
        ans += oc
    elif to == "U" and ao in ["L", "R", "D"]:
        h_check(tx, ty, ax, ay, ao, oc)
    elif to == "D" and ao in ["L", "R", "U"]:
        if ao != "U":
            h_check(ax, ty, tx, ay, ao, oc)
        else:
            h_check(ax, ty, tx, ay, "D", oc)
    elif to == "L" and ao in ["U", "D", "R"]:
        w_check(tx, ty, ax, ay, ao, oc)
    elif to == "R" and ao in ["U", "D", "L"]:
        if ao != "L":
            w_check(tx, ay, ax, ty, ao, oc)
        else:
            w_check(tx, ay, ax, ty, "R", oc)
