...
N, X = il()
A = il()
P = [p if p >= 0 else -1 for p in il(-1)]

if all([True if P[i] < i else False for i in range(N)]):
    dp = [INF] * N
    ...
    print(max([i + 1 for i in range(N) if dp[i] <= X] + [-1]))
else:
    t = [False] * N
    ans = [-1]
    for i in range(N):
        ...
        while P[current] != -1:
            S.append(current)
            if not used[P[current]] and not t[P[current]] and cost + A[P[current]] <= X:
                cost += A[P[current]]
                used[P[current]] = True
                current = P[current]
            else:
