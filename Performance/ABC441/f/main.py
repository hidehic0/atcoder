...
rdp = [[-INF for _ in [0] * (M + 1)] for _ in [0] * (N + 1)]
rdp[N][0] = 0

for i in reversed(range(N)):
    for k in range(M + 1):
        if P[i] + k <= M:
            rdp[i][P[i] + k] = max(rdp[i][P[i] + k], rdp[i + 1][k] + V[i])

        rdp[i][k] = max(rdp[i][k], rdp[i + 1][k])

fdp = create_array2(N + 1, M + 1, -INF)
fdp[0][0] = 0
mx = max(rdp[0])
ans = []
...
for i in range(N):
    ...
    seg = SegTree(max, -INF, fdp[i][:])

    for k in range(M + 1):
        if seg.prod(0, M - k + 1) + rdp[i + 1][k] == mx:
            b = True
        if (
            M - k - P[i] >= 0
            and seg.prod(0, M - k - P[i] + 1) + rdp[i + 1][k] + V[i] == mx
        ):
            a = True
    ...
    for k in range(M + 1):
        if P[i] + k <= M:
            fdp[i + 1][P[i] + k] = max(fdp[i + 1][P[i] + k], fdp[i][k] + V[i])

        fdp[i + 1][k] = max(fdp[i + 1][k], fdp[i][k])
