...
def f(mid):
    dp = create_array2(H, W, -INF)
    dp[0][0] = mid

    for i in range(H):
        for k in range(W):
            if dp[i][k] + A[i][k] >= P[i + k]:
                if i + 1 < H:
                    dp[i + 1][k] = max(dp[i + 1][k], dp[i][k] + A[i][k] - P[i + k])

                if k + 1 < W:
                    dp[i][k + 1] = max(dp[i][k + 1], dp[i][k] + A[i][k] - P[i + k])

    return dp[H - 1][W - 1] + A[H - 1][W - 1] < P[-1]


ans = binary_search(f, 200000000000100, -1, return_left=False)
