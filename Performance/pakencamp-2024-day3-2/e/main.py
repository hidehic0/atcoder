...
for _ in [0] * M:
    u, v = il(-1)
    G.new_side(u, v, max(0, H[v] - H[u]))
    G.new_side(v, u, max(0, H[u] - H[v]))

used = create_array2(N, 2, 0)
PQ = []

for i in range(N):
    heapq.heappush(PQ, (0, 0, 0, 0, i, i))

ans = 0

while PQ:
    _cos, n, score, mode, startpoint, cur = heapq.heappop(PQ)
