...
seg = SegTree(lambda x, y: min(x, y), (INF, INF, INF), N)
seg2 = SegTree(lambda x, y: max(x, y), 0, N + 1)

seg.set(0, (A[P[0]], P[0], 1))
seg2.set(P[0], 0)

t = 1

for p in P[1:]:
    mi = seg.prod(seg2.prod(p, N + 1), N)
    ind = seg.max_right(seg2.prod(p, N + 1), lambda g: g[0] > mi[0])

    cos, r, cnt = seg.get(ind)
    ans += cos
    seg.set(ind, (A[r] * ((cnt + 1) ** 2 - cnt**2), r, cnt + 1))
    seg2.set(p, t)
    seg.set(t, (A[p], p, 1))
