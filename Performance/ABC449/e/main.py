...
def chebyshev_is(x1: int, y1: int, x2: int, y2: int) -> int:
    """チェビシェフ距離を計算する関数"""
...
# 便利変数
INF = 1 << 63
lowerlist = list("abcefghijklmnopqrstuvwxyz")
...
for n in range(N + 1):
    while len(QL) and QL[-1][0] < cur + len(L):
        ans[QL[-1][1]] = L[QL[-1][0] - cur]
        QL.pop()
